...
function lock(model){walk(model,m=>m.set({draggable:false,droppable:false,copyable:false,removable:false,editable:false,resizable:false,stylable:false,selectable:!!m.getAttributes()['data-cms-fields'],hoverable:!!m.getAttributes()['data-cms-fields']}));}
...
}else models=editor.Components.addComponent(CMBlocks.html(recipe,state.site_url));
...
editor.on('canvas:frame:load',()=>{const doc=editor.Canvas.getDocument();doc.addEventListener('submit',e=>e.preventDefault());doc.addEventListener('click',e=>{if(e.target.closest('a,button'))e.preventDefault();},true);});
...
async function mutation(action,extra={}){if(busy)return false;sync();busy=true;$('#app').inert=true;try{const r=await api(action,{version:state.version,...extra});Object.assign(state,r);dirty=false;await renderPage();status(action==='publish'?'Изменения опубликованы.':action==='restore'?'Версия восстановлена в черновик.':'Черновик сохранён.');return true;}catch(e){status(e.message,true);return false;}finally{busy=false;$('#app').inert=false;}}
