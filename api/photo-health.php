...
function ph_norm(string $s): string {
    ...
    foreach($parts as $part){
        ...
        foreach($chunk as $row){
            ...
            foreach($images as $u){
                ...
                $valid[]=$u;$freq[$u]=($freq[$u]??0)+1;
            }
            if($valid)$rows[]=['name'=>ph_norm($title),'cat'=>ph_norm($cat),'images'=>array_values(array_unique($valid))];
        }
    }
    $byKey=[];$nameBuckets=[];
    foreach($rows as $row){
        $safe=[];
        foreach($row['images'] as $u){if(($freq[$u]??0)<=3)$safe[]=$u;if(count($safe)>=3)break;}
        if(!$safe)continue;
        $key=$row['name'].'|'.$row['cat'];
        if(!isset($byKey[$key]))$byKey[$key]=true;else$byKey[$key]=false;
        $nameBuckets[$row['name']][]=json_encode($safe,JSON_UNESCAPED_SLASHES);
    }
    $byKey=array_filter($byKey,fn($v)=>$v===true);
    $byName=[];
    foreach($nameBuckets as $name=>$sets){if(count(array_unique($sets))===1)$byName[$name]=true;}
    return ['by_key'=>$byKey,'by_name'=>$byName];
}
function ph_has_fallback(array $idx,string $name,string $categoryPath): bool {
    $n=ph_norm($name);if($n==='')return false;
    $parts=array_values(array_filter(array_map('trim',explode('/',$categoryPath))));
    $cat=$parts?(string)end($parts):'';$c=ph_norm($cat);
    if($c!==''&&!empty($idx['by_key'][$n.'|'.$c]))return true;
    return !empty($idx['by_name'][$n]);
}

try{
    require_admin();
    $st=$pdo->query("SELECT id,name,main_image,images,category_path FROM products WHERE is_active=1 ORDER BY id");
    $idx=ph_fallback_index();
    $stats=[
        'active_products'=>0,
        'products_without_db_image'=>0,
        'products_with_local_image'=>0,
        'products_with_remote_image'=>0,
    ...
    while($p=$st->fetch(PDO::FETCH_ASSOC)){
        ...
        if(!$urls)$stats['products_without_db_image']++;
        $hasLocal=false;$hasRemote=false;$hadBrokenLocal=false;
        foreach($urls as $u){
            $s=ph_local_status((string)$u);
            if($s===true)$hasLocal=true;
            elseif($s===false){$stats['broken_local_references']++;$hadBrokenLocal=true;}
            else $hasRemote=true;
        }
        if($hasLocal)$stats['products_with_local_image']++;
        if($hasRemote)$stats['products_with_remote_image']++;
        if($hadBrokenLocal&&!$hasLocal&&!$hasRemote)$stats['products_with_broken_local_only']++;
        $needsFallback=!$hasLocal&&!$hasRemote;
        $fallback=$needsFallback&&ph_has_fallback($idx,(string)$p['name'],(string)($p['category_path']??''));
        if($fallback)$stats['fallback_available']++;
        if($needsFallback&&!$fallback){
            $stats['unresolved_products']++;
            if(count($examples)<20)$examples[]=['id'=>(int)$p['id'],'name'=>(string)$p['name'],'category'=>(string)($p['category_path']??'')];
        }
    }
