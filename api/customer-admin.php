...
try{
  ...
  if(($_GET['export']??'')==='csv'){
    $rows=customer_rows($pdo,$q,10000);header('Content-Type: text/csv; charset=utf-8');header('Content-Disposition: attachment; filename="profisport-clients-'.date('Y-m-d').'.csv"');header('Cache-Control: no-store');echo "\xEF\xBB\xBF";$stream=fopen('php://output','wb');fputcsv($stream,['ID','Имя','Фамилия','Телефон','Email','Дата рождения','Источник','Есть кабинет','Дата QR-регистрации','Дата добавления'],';');foreach($rows as $row)fputcsv($stream,[$row['id'],$row['name'],$row['last_name'],$row['phone'],$row['email'],$row['birth_date'],$row['registration_source'],(int)$row['has_account']?'да':'нет',$row['qr_registered_at'],$row['created_at']],';');fclose($stream);exit;
  }
