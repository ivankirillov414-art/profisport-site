...
try{
  ...
  if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action!=='save_draft')out(['ok'=>false,'error'=>'bad_action'],422);
  ...
  out(['ok'=>true,'program'=>$status,'stats'=>$stats,'category_options'=>loyalty_category_options($pdo),'csrf'=>$_SESSION['csrf']??'','can_edit'=>($admin['role']??'')==='owner','live_activation_available'=>false]);
