...
try{
  $stmt=$pdo->query("SELECT id,source_id,name,slug,sku,brand,model,price,old_price,stock_status,stock_qty,short_description,description,specs_json,main_image,source_hash,external_url,price_rub,old_price_rub,availability,category_path,specs,images,updated_at FROM products WHERE is_active=1 ORDER BY sort_order ASC,id ASC");
  $items=[];
  while($p=$stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    $items[]=[
      ...
      'availability'=>$p['availability']?:$p['stock_status'],'stock_status'=>$p['stock_status'],'stock_qty'=>$p['stock_qty'],'category_path'=>$categoryPath,
