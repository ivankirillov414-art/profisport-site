...
try{
  ...
  if($relatedTo>0){
    $sql=str_replace(' ORDER BY'," AND id<>".$relatedTo." AND category_path<>'' AND category_path=(SELECT related_source.category_path FROM products related_source WHERE related_source.id=".$relatedTo.") ORDER BY",$sql);
    $limit=12;
    $offset=0;
  }
  ...
  while($p=$stmt->fetch()){
    ...
    $items[]=[
      ...
      'description'=>$productId>0?($p['description']?:($p['short_description']??'')):($p['short_description']?:($p['description']??'')),
      'specs'=>$specs,
      'images'=>$images,
      'image'=>$images[0]??null,
      'fallback_image'=>null,
