...
const visibility=['mobile','desktop'].includes(p.visibility)?' cms-only-'+p.visibility:'',fonts={Arial:'Arial, sans-serif',Verdana:'Verdana, sans-serif',Georgia:'Georgia, serif',Trebuchet:'Trebuchet MS, sans-serif',Times:'Times New Roman, serif'};
return `<section class="cms-block${visibility}" data-cms-new="${esc(block.id)}" style="background:${color(p.background,'#ffffff')};color:${color(p.color,'#172033')};text-align:${align};padding-top:${space}px;padding-bottom:${space}px;font-family:${fonts[p.font]||fonts.Arial};font-size:${/^[1-9][0-9]?$/.test(p.fontSize)?p.fontSize+'px':'inherit'};font-weight:${['400','500','600','700','800'].includes(p.fontWeight)?p.fontWeight:'inherit'};--cms-mobile-space:${mobile}px;--cms-columns:${cols};--cms-radius:${radius}px;--cms-accent:${color(p.accent,'#2463eb')}"><div class="cms-inner">${inner}</div></section>`;
}
