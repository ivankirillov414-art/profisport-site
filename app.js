...
function render(list=view){
  ...
  catalogStatus.textContent=`${view.length} товаров`;if($('#mobileSort'))$('#mobileSort').value=sort.value;if($('#filterDialog').open)$('#applyFilters').textContent=`Показать товары (${view.length})`;pageInfo.textContent=`${page} / ${pages}`;prevPage.disabled=!catalogComplete||page<=1;nextPage.disabled=!catalogComplete||page>=pages;
  if(!catalogComplete){catalogStatus.textContent=show.length?'Первые товары · загружаем остальные…':'Загружаем каталог…';pageInfo.textContent='…';if($('#filterDialog').open)$('#applyFilters').textContent='Показать';}
}
...
function apply(resetPage=true,sync=true){
  ...
  if(typeof applyExtraFilters==='function')list=applyExtraFilters(list);
...
function selectDepartment(key){
  category.value=key;if(typeof resetExtraFilters==='function')resetExtraFilters();selectedSubcategory='';q.value='';mobileQ.value='';brandFilter.value='';facet1.value='';facet2.value='';apply();$('#catalogProducts').scrollIntoView({behavior:'smooth'});
}
...
function renderFilterChips(){
  ...
  if(typeof extraFilterState!=='undefined')for(const [key,value] of extraFilterState)chips.push(['spec_'+key,EXTRA_SPEC_DEFS.find(d=>d[0]===key)[1]+': '+value]);
  ...
  $$('#activeFilters button').forEach(b=>b.onclick=()=>{
    ...
    if(id.startsWith('spec_')&&typeof extraFilterState!=='undefined'){extraFilterState.delete(id.slice(5));apply();return}
    if(id==='all'){$('#resetFilters').click();return}
    if(id==='query'){q.value='';mobileQ.value=''}if(id==='category'){category.value='';selectedSubcategory=''}if(id==='subcategory')selectedSubcategory='';if(id==='sale')saleOnly.checked=false;
...
function setupHero(){
  ...
  slider.addEventListener('pointermove',e=>{
    ...
    if(!gesture.dragging){
      if(Math.abs(dy)>8&&Math.abs(dy)>Math.abs(dx)){gesture=null;resume();return}
  ...
  const finishGesture=e=>{
    if(!gesture||gesture.id!==e.pointerId)return;
    const {dx,dragging}=gesture;gesture=null;slider.classList.remove('is-dragging');
    if(slider.hasPointerCapture(e.pointerId))slider.releasePointerCapture(e.pointerId);
    suppressClick=dragging;setTimeout(()=>{suppressClick=false},0);
    setSlide(current+(e.type==='pointerup'&&dragging&&Math.abs(dx)>45?(dx<0?1:-1):0));
  };
  window.addEventListener('pointerup',finishGesture);
  window.addEventListener('pointercancel',finishGesture);
  slider.addEventListener('lostpointercapture',finishGesture);
  slider.addEventListener('click',e=>{if(suppressClick){e.preventDefault();e.stopPropagation()}},true);
  window.addEventListener('resize',()=>{if(gesture)finishGesture({pointerId:gesture.id,type:'pointercancel'});setSlide(current)},{passive:true});
...
function renderPickerResults(matches,budget){const box=$('#pickerResults');if(!box)return;if(!matches.length){box.innerHTML='<p class="pickerEmpty">В этом бюджете сейчас нет велосипедов в наличии. Увеличьте бюджет или посмотрите весь каталог.</p>';return}const shown=matches.slice(0,4),allLink=`?cat=bicycle&max=${encodeURIComponent(budget)}#catalogProducts`;box.innerHTML=`<div class="pickerResultHead"><h3>Подходящие велосипеды</h3><span>Показано ${shown.length} из ${matches.length}</span></div>`+shown.map(product=>{const image=imageCandidates(product)[0],photo=image?`<img src="${esc(image)}" alt="${esc(product.name)}" loading="lazy">`:'<span class="pickerResultPhoto">Фото уточняется</span>';return `<article class="pickerResultCard"><a href="product.html?id=${encodeURIComponent(product.id)}">${photo}<b>${esc(product.name)}</b><strong>${rub(product.price)}</strong></a></article>`}).join('')+`<a class="pickerResultAll" href="${allLink}">Открыть все ${matches.length} велосипедов в каталоге →</a>`}
$('#pick')?.addEventListener('click',()=>{const result=$('#pickResult'),box=$('#pickerResults');if(!catalogComplete){result.textContent='Каталог ещё загружается. Подбор станет доступен после загрузки.';box.innerHTML='';return}const height=+$('#height').value,budget=+$('#budget').value,ride=$('#ride').value;if(!height||!budget){result.textContent='Укажите рост и бюджет.';box.innerHTML='';return}const matches=products.filter(product=>isPrimaryProduct(product,'bicycle')&&product.price>0&&product.price<=budget&&product.stockCode!=='out').sort((a,b)=>pickerRideScore(b,ride)-pickerRideScore(a,ride)||a.price-b.price);result.innerHTML=`Ориентир размера рамы: <b>${pickerFrameForHeight(height)}</b>. Для «${esc(ride)}» найдено: <b>${matches.length}</b>.`;renderPickerResults(matches,budget)});
...
(async()=>{
  try{
    ...
    const all=await loadRealCatalog(initial=>{
      if(catalogComplete)return;
      products=initial;render(hasPendingFilters()?[]:products);
    });
    if(!all.length)throw Error('empty');products=all;catalogComplete=true;
