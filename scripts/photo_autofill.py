...
def row_category(row):
 p=row.get('category_path') or row.get('breadcrumbs') or [];return ' / '.join(str(x) for x in p if str(x).strip()) if isinstance(p,list) else str(p or '')
...
def confidence(title,name,brand,model,category):
 ...
 if target_nums and cand_nums and not target_nums.issubset(cand_nums):return 0
 ...
 if model and (not model_match or cov<0.34):return 0
 if not model and cov<0.68 and tn!=nn and nn not in tn:return 0
 score=(100 if tn==nn else 0)+(55 if nn in tn or tn in nn else 0)+(20 if brand_match else 0)+(45 if model_match else 0)+common*22+int(cov*20)
 if cov>=.8:score+=25
 if target_nums and target_nums.issubset(cand_nums):score+=30
...
def image_usable(url):
 ...
 try:
  ...
  with urllib.request.urlopen(req,timeout=8) as r:
   if (r.headers.get('Content-Type') or '').lower().startswith('image/'):return True
 except Exception:pass
 return bool(IMG_EXT.search(urllib.parse.urlparse(url).path))
def page_image(url,expected_name='',brand='',model='',category='',trust_source=False):
 ...
 if not trust_source and score<82:return None
 for raw in p.images:
...
def bing_image_candidate(query,name,brand,model,category):
 ...
 for img,src,title in p.items[:80]:
  ...
  seen.add(img);considered+=1;score=confidence(title or src,name,brand,model,category)
  if score<85 or not image_usable(img):continue
...
def web_candidate(name,brand,model,category):
 query=' '.join(str(x).strip() for x in [name,category,brand,model] if str(x or '').strip());got,n=bing_image_candidate(query,name,brand,model,category)
 if got:return got,n
 seen=set()
 for engine in (bing_results,ddg_results):
  for u,t in engine(query):
   ...
   seen.add(u)
   if confidence(t,name,brand,model,category)<80:continue
...
def main():
 ...
 for _,name,r in batch:
  ...
  if not got and searched<args.max_search:
   searched+=1;got,n=web_candidate(name,brand,model,cat);bing_items+=n;web_hits+=1 if got else 0
  if not got:continue
  img=got['image']
