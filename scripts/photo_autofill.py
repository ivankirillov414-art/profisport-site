...
OUT=os.path.join(DATA,'photo-overrides.json')
...
def distinctive(s,brand=''):
    bt=set(tokens(brand))
    return [x for x in tokens(s) if x not in GENERIC and x not in bt]


def numbers(s):
    return {x.lstrip('0') or '0' for x in re.findall(r'(?<![\w])\d+(?:[.,]\d+)?(?![\w])',str(s or ''),flags=re.U)}
...
def row_category(row):
    ...
    if isinstance(p,list):return ' / '.join(str(x) for x in p if str(x).strip())
    return str(p or '')
...
def confidence(title,name,brand,model,category):
    ...
    if not tn or not nn:return 0
    if any(x in tn for x in BAD_PAGE_WORDS):return 0
    target_nums=numbers(name+' '+model)
    cand_nums=numbers(title)
    if target_nums and cand_nums and not target_nums.intersection(cand_nums):return 0
    brand_match=bool(bn and bn in tn)
    model_match=bool(mn and len(mn)>=2 and mn in tn)
    if bn and not brand_match and not model_match:return 0
    td=distinctive(name+' '+model,brand)
    cd=set(distinctive(title,brand))
    common=sum(1 for x in td if x in cd)
    cov=common/max(1,len(td))
    score=0
    if tn==nn:score+=100
    if nn in tn or tn in nn:score+=55
    if brand_match:score+=20
    if model_match:score+=45
    score+=int(cov*45)
    if target_nums:
        score+=min(20,10*len(target_nums.intersection(cand_nums)))
    cat_tokens=set(distinctive(category))
    if cat_tokens and cat_tokens.intersection(set(tokens(title))):score+=5
    if model:
        if not model_match:return 0
        if cov<0.34:return 0
    elif cov<0.72 and tn!=nn and nn not in tn:
        return 0
...
class MetaParser(HTMLParser):
    def __init__(self):
        super().__init__();self.images=[];self.title='';self._in_title=False
    def handle_starttag(self,tag,attrs):
        ...
        if tag.lower()=='link' and d.get('itemprop','').lower()=='image' and d.get('href'):
...
def request(url,timeout=12,limit=1_500_000):
...
def image_usable(url):
    ...
    low=url.lower()
    if any(x in low for x in BAD_IMAGE_WORDS):return False
    try:
...
def page_image(url,expected_name='',brand='',model='',category='',trust_source=False):
    ...
    score=100 if trust_source else confidence(title,expected_name,brand,model,category)
    if not trust_source and score<80:return None
    for raw in p.images:
        img=urllib.parse.urljoin(final,html.unescape(raw.strip()))
        if image_usable(img):
            return {'image':img,'source_url':final,'source_title':title or expected_name,'confidence':score,'method':'source-page' if trust_source else 'web-search'}
    return None
...
def web_candidate(name,brand,model,category):
    parts=[name,category,brand,model]
    query=' '.join(x.strip() for x in parts if str(x or '').strip())
    seen=set()
...
def main():
    ...
    ap.add_argument('--max-search',type=int,default=20)
    args=ap.parse_args()
    state=load_overrides();items=state['items'];rows=load_rows()
    existing_keys=set(norm(k) for k in items)
    ...
    for r in rows:
        ...
        stock=str(r.get('availability') or r.get('stock_status') or '')
        qty=r.get('stock_qty')
        priority=2 if stock=='in_stock' else (1 if isinstance(qty,(int,float)) and qty>0 else 0)
        pending.append((priority,name,r))
    pending.sort(key=lambda x:(-x[0],x[1]))
    added=0;searched=0;source_hits=0;web_hits=0
    for _,name,r in pending[:max(args.max_products,1)]:
        key=norm(name);brand=str(r.get('brand') or '');model=str(r.get('model') or '');cat=row_category(r)
        got=None;src=source_url(r)
        if src:
            got=page_image(src,name,brand,model,cat,True)
            if got:source_hits+=1
        if not got and searched<args.max_search:
            searched+=1;got=web_candidate(name,brand,model,cat)
            if got:web_hits+=1
        if not got:continue
        img=got['image']
        if image_counts[img]>=3:continue
        image_counts[img]+=1
        items[key]={'name':name,'image':img,'source_url':got.get('source_url',''),'source_title':got.get('source_title',''),'method':got.get('method',''),'confidence':int(got.get('confidence',0))}
        existing_keys.add(key);added+=1
        print(f"ADD {name} -> {img}")
        time.sleep(.35)
    state['generated_at']=datetime.now(timezone.utc).isoformat()
    state['items']=dict(sorted(items.items()))
    state['stats']={'catalog_rows':len(rows),'pending_without_source_image':len(pending),'added_this_run':added,'source_page_hits':source_hits,'web_search_hits':web_hits,'web_searches':searched,'total_overrides':len(items)}
