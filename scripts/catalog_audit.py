...
TYPE_RULES=[
 ...
 ('rollers',re.compile(r'^(?:роликовые коньки|ролики)(?:\s|-)',re.I),('ролик',)),
 ('skis',re.compile(r'^лыжи(?:\s|-)',re.I),('лыж',)),
 ('skateboard',re.compile(r'^скейтборд(?:\s|-)',re.I),('скейт',)),
 ('longboard',re.compile(r'^лонгборд(?:\s|-)',re.I),('лонгборд',)),
...
def main():
 ...
 for i,row in enumerate(rows):
  ...
  brand=str(row.get('brand') or '').strip() or str(specs.get('Бренд') or '').strip()
  ...
  if kind:
   ...
   if stems and not any(s in ptext for s in stems):
    issues.append({'kind':'category_conflict','type':kind,'name':name,'category_path':path,'url':u})
  if kind!='bicycle' and 'ростовка рамы' in {norm(k) for k in specs}:
   v='';
   for k,val in specs.items():
    if norm(k)=='ростовка рамы':v=str(val);break
   issues.append({'kind':'suspicious_spec','type':kind or 'unknown','name':name,'spec':'Ростовка рамы','value':v,'category_path':path,'url':u})
