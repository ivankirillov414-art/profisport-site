...
for(const table of ['products','customers','orders','loyalty_transactions','customer_vehicles','service_requests','service_request_status_history','vehicle_components','vehicle_component_events']){
...
for(const table of ['loyalty_transactions','customer_vehicles','service_request_status_history','vehicle_components','vehicle_component_events']) assert.ok(adminHealth.includes("'"+table+"'"),'admin health missing '+table);
...
for(const check of ['profile-phone-login-regression.js','customer-phone-login-regression.js','admin-customer-card-regression.js','vehicle-passport-regression.js','loyalty-live-regression.js','production-preflight-regression.js','tests/loyalty-live.php','tests/vehicle-passport.php','tests/phone-login-http.py']){
  assert.ok(deploy.includes(check),'deploy gate missing '+check);
}
for(const deployed of ['404.html','500.html','admin/customer.php','admin/vehicle.php','api/vehicle-passport-admin.php','server/vehicle-passport.php']) assert.ok(deploy.includes(deployed),'FTP verification missing '+deployed);
