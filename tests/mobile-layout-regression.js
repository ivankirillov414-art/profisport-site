...
if(/100vw/i.test(css))fail('storefront-v2.css must not use 100vw');
...
if(!/select\{[^}]*-webkit-appearance:none;[^}]*inline-size:100%;[^}]*min-inline-size:0/.test(css))fail('native selects must not contribute their option intrinsic width');
if(!/#catalogProducts\{[^}]*contain:inline-size/.test(css))fail('catalog must contain intrinsic inline sizing');
if(!/\.filters\{[^}]*contain:inline-size/.test(css))fail('filter row must contain intrinsic inline sizing');
