...
function ensure_product_columns(PDO $pdo): void {
  ...
  try{$idx=[];foreach($pdo->query("SHOW INDEX FROM products") as $r)$idx[(string)$r['Key_name']]=true;if(!isset($idx['idx_source_hash']))$pdo->exec('CREATE INDEX idx_source_hash ON products(source_hash)');if(!isset($idx['idx_active']))$pdo->exec('CREATE INDEX idx_active ON products(is_active)');}catch(Throwable $e){error_log('index_migration_failed: '.$e->getMessage());}
}
...
$pdo->exec("CREATE TABLE IF NOT EXISTS products (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,source_hash CHAR(64) NULL,external_url VARCHAR(1000) NULL,title VARCHAR(500) NOT NULL,price_rub INT NOT NULL DEFAULT 0,old_price_rub INT NULL,availability VARCHAR(40) NOT NULL DEFAULT 'unknown',category_path TEXT NULL,specs LONGTEXT NULL,description LONGTEXT NULL,images LONGTEXT NULL,is_active TINYINT(1) NOT NULL DEFAULT 1,created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,INDEX idx_source_hash (source_hash),INDEX idx_title (title(191)),INDEX idx_price (price_rub),INDEX idx_active (is_active)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
