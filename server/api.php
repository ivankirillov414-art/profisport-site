...
try {
    ...
    if ($action === 'setup' && $_SERVER['REQUEST_METHOD']==='POST') {
        $in=input_json(); $token=(string)($in['setup_token']??''); $new=(string)($in['new_password']??'');
        if (time() > SETUP_TOKEN_EXPIRES) json_response(['ok'=>false,'error'=>'setup_token_expired'],403);
        if ($token==='' || !hash_equals(SETUP_TOKEN_HASH,hash('sha256',$token))) json_response(['ok'=>false,'error'=>'setup_token'],403);
        ...
        try {
            ...
            $pdo->commit();
            audit($pdo,'admin_setup','admin_user',(string)$u['id']);
    ...
    if ($action === 'login' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        session_regenerate_id(true); $_SESSION['admin']=['id'=>(int)$u['id'],'username'=>$u['username'],'role'=>$u['role']]; $_SESSION['csrf']=bin2hex(random_bytes(24)); audit($pdo,'login','admin_user',(string)$u['id']); json_response(['ok'=>true,'admin'=>$_SESSION['admin'],'csrf'=>$_SESSION['csrf']]);
    }
    if ($action === 'logout' && $_SERVER['REQUEST_METHOD']==='POST') { require_admin(); csrf_check(); audit($pdo,'logout'); $_SESSION=[]; session_destroy(); json_response(['ok'=>true]); }
    ...
    if ($action === 'products' && $_SERVER['REQUEST_METHOD']==='GET') {
        ...
        if ($q!=='') { $s=$pdo->prepare('SELECT * FROM products WHERE title LIKE ? ORDER BY id DESC LIMIT '.$limit.' OFFSET '.$offset); $s->execute(['%'.$q.'%']); } else $s=$pdo->query('SELECT * FROM products ORDER BY id DESC LIMIT '.$limit.' OFFSET '.$offset);
        $rows=$s->fetchAll(); foreach($rows as &$r){$r['images']=json_decode($r['images']?:'[]',true)?:[];$r['specs']=json_decode($r['specs']?:'{}',true)?:[];} json_response(['ok'=>true,'items'=>$rows,'page'=>$page]);
    }
    if ($action === 'product_save' && $_SERVER['REQUEST_METHOD']==='POST') {
        require_admin(); csrf_check(); $in=input_json(); $id=(int)($in['id']??0); if($id<1)json_response(['ok'=>false,'error'=>'bad_id'],422);
        $s=$pdo->prepare('UPDATE products SET title=?,price_rub=?,old_price_rub=?,availability=?,category_path=?,description=?,is_active=? WHERE id=?'); $s->execute([trim((string)$in['title']),max(0,(int)$in['price_rub']),($in['old_price_rub']===''||$in['old_price_rub']===null)?null:max(0,(int)$in['old_price_rub']),trim((string)($in['availability']??'unknown')),trim((string)($in['category_path']??'')),(string)($in['description']??''),!empty($in['is_active'])?1:0,$id]); audit($pdo,'product_save','product',(string)$id); json_response(['ok'=>true]);
    }
    ...
    if ($action === 'import_batch' && $_SERVER['REQUEST_METHOD']==='POST') {
        ...
        foreach($items as $p){$url=(string)($p['url']??'');$title=trim((string)($p['title']??''));if($title==='')continue;$hash=hash('sha256',$url!==''?$url:$title);$s->execute([$hash,$url,$title,max(0,(int)($p['price_rub']??0)),isset($p['old_price_rub'])?(int)$p['old_price_rub']:null,(string)($p['availability']??'unknown'),is_array($p['category_path']??null)?implode(' > ',$p['category_path']):(string)($p['category_path']??''),json_encode($p['specs']??[],JSON_UNESCAPED_UNICODE),$p['description']??'',json_encode($p['images']??[],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)]);$n++;} audit($pdo,'import_batch','product',null,['count'=>$n]); json_response(['ok'=>true,'imported'=>$n]);
    }
