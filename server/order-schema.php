...
function ensure_order_columns(PDO $pdo): void {
    ...
    if((int)$pdo->query("SELECT GET_LOCK('profisport_order_columns_v2',10)")->fetchColumn()!==1)throw new RuntimeException('Order schema migration is busy');
    try{
        $done->execute([$version]);if($done->fetchColumn()==='1')return;
        $definitions=[
            'orders'=>['delivery_method'=>'VARCHAR(30) NULL','pickup_store'=>'VARCHAR(180) NULL','total_rub'=>'DECIMAL(12,2) NULL'],
