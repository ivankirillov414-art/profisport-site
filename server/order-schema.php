...
function ensure_order_columns(PDO $pdo): void {
    ...
    if((int)$pdo->query("SELECT GET_LOCK('profisport_order_columns_v4',10)")->fetchColumn()!==1)throw new RuntimeException('Order schema migration is busy');
    try{
        $done->execute([$version]);if($done->fetchColumn()==='1')return;
        $definitions=[
            'orders'=>['delivery_method'=>'VARCHAR(30) NULL','pickup_store'=>'VARCHAR(180) NULL','subtotal_rub'=>'DECIMAL(12,2) NULL','discount_rub'=>'DECIMAL(12,2) NOT NULL DEFAULT 0','total_rub'=>'DECIMAL(12,2) NULL','bonus_spent'=>'INT NOT NULL DEFAULT 0','bonus_earned'=>'INT NOT NULL DEFAULT 0','payable_rub'=>'DECIMAL(12,2) NULL'],
            'order_items'=>['title'=>'VARCHAR(500) NULL','base_price_rub'=>'DECIMAL(12,2) NULL','price_rub'=>'DECIMAL(12,2) NULL','discount_percent_bp'=>'INT NOT NULL DEFAULT 0','discount_rub'=>'DECIMAL(12,2) NOT NULL DEFAULT 0','base_line_total_rub'=>'DECIMAL(12,2) NULL','line_total_rub'=>'DECIMAL(12,2) NULL','category_path'=>'TEXT NULL'],
        ];
        ...
        $pdo->exec("UPDATE orders SET subtotal_rub=COALESCE(subtotal_rub,total_rub),payable_rub=COALESCE(payable_rub,total_rub),updated_at=updated_at WHERE subtotal_rub IS NULL OR payable_rub IS NULL");
        ...
        $pdo->exec("UPDATE order_items SET title=COALESCE(title,$title),price_rub=COALESCE(price_rub,$price),base_price_rub=COALESCE(base_price_rub,price_rub,$price),line_total_rub=COALESCE(line_total_rub,$line),base_line_total_rub=COALESCE(base_line_total_rub,line_total_rub,$line) WHERE title IS NULL OR price_rub IS NULL OR base_price_rub IS NULL OR line_total_rub IS NULL OR base_line_total_rub IS NULL");
    ...
    }finally{$pdo->query("SELECT RELEASE_LOCK('profisport_order_columns_v4')");}
}
